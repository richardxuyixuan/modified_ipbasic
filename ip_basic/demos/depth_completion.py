...
def main():
    ...
    input_depth_dir = os.path.expanduser(
        '/home/user/Documents/carla_data/pncnn/pncnn_input/valid/nusc_velodyne')
    data_split = 'val'
    ...
    for i in range(num_images):
        ...
        projected_depths = np.load(depth_image_path)[0]
        ...
        for layer_index in range(projected_depths.shape[0]):
            if fill_type == 'fast':
                final_depth = depth_map_utils.fill_in_fast(
                    projected_depths[layer_index], extrapolate=extrapolate, blur_type=blur_type)
            ...
            if final_depths is not None:
                final_depths = np.concatenate((final_depths, np.expand_dims(final_depth,axis=0)),axis=0)
            else:
                final_depths = np.expand_dims(final_depth,axis=0)
        ...
        if save_depth_maps:
            ...
            final_depths[0] *= 1.6433 + 2.1008
            final_depths[1] *= 6.2918 + 6.1954
            final_depths[2] *= 7.0150 + 6.9538
            final_depths[3] *= 4.9339 + 12.7296
            final_depths[4] *= 0.6881 + 0.3313
            final_depths[0] -= 1.6433
            final_depths[1] -= 6.2918
            final_depths[2] -= 7.0150
            final_depths[3] -= 4.9339
            final_depths[4] -= 0.6881
